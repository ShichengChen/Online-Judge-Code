...
public:
    ...
    friend void read(modnum& n){ll v_; cin >> v_; n = modnum(v_);}
...
bool KMP(vt<int>& pattern,vt<int>& text,int st){
    int m = sz(pattern);
    vector<int>f(m+1);
    get_fail(pattern,f);
    for (int i = st,j=0;i< sz(text) ; ++i) {
...
void solve(){
    ...
    read(n,k);
    vt<int> arr(n);
    vt<int> brr(n*2-1),crr(n);
    read(arr,crr);
    FOR(i,2*n-1)brr[i]=crr[i%n];
    ...
    if(k==0){
        ...
        if(KMP(arr,brr,1))print("YES");
        else print("NO");
    }
    else{
        if(KMP(arr,brr,0))print("YES");
        else print("NO");
    }
}
int main() {
...
//    freopen("/home/csc/Downloads/consecutive_cuts_chapter_2_input (1).txt", "r", stdin);
