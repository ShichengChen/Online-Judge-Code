...
void solve(){
    ...
    vt<int>B(m);
    read(B);
    vt<vt<int>> d(n, vt<int>(m,2e8));
    vt<vt<int>>vec(2600);
    FOR(i, 2501)FOR(j,sz(B))if(i==B[j])vec[i].push_back(j);
    FOR(j,m)d[0][j]=0;
    FOR(i,1,n)EACH(idx0,vec[A[i-1]]){

            int id= lower_bound(all(vec[A[i]]),idx0)-vec[A[i]].begin();
//            debug(i,idx0,id);
            if(id== sz(vec[A[i]]))id--;
            else umin(d[i][vec[A[i]][id]],d[i-1][idx0]+abs(idx0-vec[A[i]][id]));
            if(id)id--;
            umin(d[i][vec[A[i]][id]],d[i-1][idx0]+abs(idx0-vec[A[i]][id]));
        }
    print(d);
    print(*min_element(all(d[n-1])));
}
int main() {
