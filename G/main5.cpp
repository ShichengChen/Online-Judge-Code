...
bool f(ll v){
    for (int i = 0,j=0; i < m; ++i) {
        if(vec[1][i]>=vec[0][j] && vec[1][i]-vec[0][j]>=v){i++;continue;}
        while(j<n && vec[1][i]>=vec[0][j] && vec[1][i]-vec[0][j]<v)j++;
        if(j==n)return false;
        if(vec[1][i]<vec[0][j]){
            if(vec[0][j]-vec[1][i]>v)return false;
            else{
                ll extra=v-(vec[0][j]-vec[1][i])/2;
                int idx=lower_bound(all(vec[1]),extra+vec[0][j])-vec[1].begin();
                if(idx==sz(vec[1]))return true;
                if(vec[1][idx]>extra+vec[0][j])idx--;
                i=idx;j++;
            }
        }
    }
...
//    cin.tie(0);
//    freopen("/home/csc/Downloads/perimetric_chapter_3_input.txt", "r", stdin);
